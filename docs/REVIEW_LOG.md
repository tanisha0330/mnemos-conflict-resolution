...
1. **[FIXED 2026-08-15] The flagship demo conflict sometimes never fired at all.** Was: Stage 1's real cosine similarity between "refund is pending" and "refund was processed" for the same order measured **0.49 and 0.54** across two pairs, straddling the spec'd 0.5 no-conflict threshold. Fixed by lowering `NO_CONFLICT_THRESHOLD` to 0.4 after re-measuring the real distribution (25 live runs: min 0.4997 post-fix, 10/10 correctly detected) and confirming via a corrected stress test that this doesn't raise the real false-positive rate (13.8% before and after — the first stress-test attempt showed 24.4%, which turned out to be the test's own synthetic-data generator using a stale hardcoded range, not a real effect). Full detail in the 2026-08-15 entry below. **Still worth spot-checking live before demo day** — this closes the measured gap, not a formal guarantee for every possible paraphrase.
...
**Left running, deliberately, per the task's own final instruction** ("redeploy once from scratch" implies the end state is deployed): the stack is live, including the EventBridge rule polling every 1 minute against the real shared database. Flagged prominently in `infra/README.md`'s new "Operational note" section and the top-of-file summary above — this will keep mutating shared/demo data unattended for as long as it stays deployed, worth a deliberate decision before demo day (scale the schedule down, disable the rule, or `cdk destroy` between verification sessions).
