...
def extract_claim_text(raw_text: str, client=None, model_id: str | None = None) -> str:
    ...
    prompt = f"""Extract exactly one clear factual claim from the following text, as a single plain sentence, in this exact form: "<Subject> <verb phrase stating the fact>" (e.g. "Order-12345's refund was processed", "Customer 4471's shipping address is 123 Main St", "Order-98765 shipped via FedEx, tracking 794658312"). Always rewrite into this normalized form - never pass through source jargon, field names, or event names verbatim (e.g. a raw "refund.processed" webhook event must become "was processed", not be kept as "refund.processed"; a "Zendesk ticket" or "Stripe webhook" mention describes the source, not the claim, so drop it from the sentence itself). Keep every concrete detail from the source text that identifies or specifies the fact - carrier names, tracking numbers, amounts, dates, addresses, statuses. Normalizing how a detail is phrased is fine; dropping it is not - a shorter sentence that loses a real detail is wrong, not "concise". Don't add hedging, don't add commentary, don't quote the source.
